...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    // Do NOT pass Error object to cb to avoid TS typing issues here
    cb(null, ok);
  }
});
...
// Get current user profile (incl. avatar url)
...
usersRoutes.post("/me/avatar", upload.single("avatar"), async (req: AuthedRequest, res) => {
  const userId = req.userId!;
  const file = req.file;
  if (!file) return res.status(400).json({ message: "No file uploaded or invalid file type" });
